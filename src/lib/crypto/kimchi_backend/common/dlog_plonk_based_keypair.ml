...
module type Inputs_intf = sig
  ...
  module Rounds : Kimchi_backend_types.Nat.Intf
...
module Make (Inputs : Inputs_intf) = struct
  ...
  let name =
    sprintf "%s_%d_v4" Inputs.name
      (Kimchi_backend_types.Nat.to_int Inputs.Rounds.n)

  let set_urs_info, load_urs =
    let urs_info = Set_once.create () in
    let urs = ref None in
    let degree = 1 lsl Kimchi_backend_types.Nat.to_int Inputs.Rounds.n in
  ...
  let array_to_vector a = Kimchi_backend_types.Vector.of_list (Array.to_list a)
  ...
  let vk_commitments (t : Inputs.Verifier_index.t) :
    ...
    in
    { sigma_comm =
        Kimchi_backend_types.Vector.init Plonk_types.Permuts.n ~f:(fun i ->
            g t.evals.sigma_comm.(i) )
    ; coefficients_comm =
        Kimchi_backend_types.Vector.init Plonk_types.Columns.n ~f:(fun i ->
  ...
  let full_vk_commitments (t : Inputs.Verifier_index.t) :
    ...
    ; lookup_table_comm =
        Kimchi_backend_types.Vector.init Plonk_types.Lookup_sorted_minus_1.n
          ~f:(fun i ->

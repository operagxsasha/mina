...
module Make (Inputs : Inputs_intf) = struct
  ...
  let opening_proof_of_backend_exn (t : Opening_proof_backend.t) =
    let g (x : G.Affine.Backend.t) : G.Affine.t =
      G.Affine.of_backend x |> Kimchi_backend_types.Or_infinity.finite_exn
    in
  ...
  let of_backend (t : Backend.t) : t =
    ...
    in
    let w_comm =
      tuple15_to_vec t.commitments.w_comm
      |> Kimchi_backend_types.Vector.map ~f:wo
    in
    create
      ~messages:
        { w_comm
  ...
  let to_backend' (chal_polys : Challenge_polynomial.t list) primary_input
      ({ messages = { w_comm; z_comm; t_comm; lookup }
       ; openings =
        ...
        t ) : Backend.t =
    let g x = G.Affine.to_backend (Kimchi_backend_types.Or_infinity.Finite x) in
    let pcwo t = Poly_comm.to_backend (`Without_degree_bound t) in
    let lr = Array.map lr ~f:(fun (x, y) -> (g x, g y)) in
    { commitments =
        { w_comm =
            tuple15_of_vec (Kimchi_backend_types.Vector.map ~f:pcwo w_comm)
        ; z_comm = pcwo z_comm
        ; t_comm = pcwo t_comm
        ; lookup =

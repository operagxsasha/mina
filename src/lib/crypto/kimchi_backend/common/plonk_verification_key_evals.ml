...
let map
  ...
  { sigma_comm = Kimchi_backend_types.Vector.map ~f sigma_comm
  ; coefficients_comm = Kimchi_backend_types.Vector.map ~f coefficients_comm
...
let map2 t1 t2 ~f =
  { sigma_comm = Kimchi_backend_types.Vector.map2 ~f t1.sigma_comm t2.sigma_comm
  ; coefficients_comm =
      Kimchi_backend_types.Vector.map2 ~f t1.coefficients_comm
...
let typ g =
  Kimchi_pasta_snarky_backend.Step_impl.Typ.of_hlistable
    [ Kimchi_backend_types.Vector.typ g Plonk_types.Permuts.n
    ; Kimchi_backend_types.Vector.typ g Plonk_types.Columns.n
    ; g
    ; g
    ; g
    ; g
...
module Step = struct
  ...
  let map
    ...
    ; lookup_table_comm =
        Kimchi_backend_types.Vector.map ~f:f_opt lookup_table_comm
  ...
  let map2 t1 t2 ~f ~f_opt =
    { sigma_comm =
        Kimchi_backend_types.Vector.map2 ~f t1.sigma_comm t2.sigma_comm
    ; coefficients_comm =
        Kimchi_backend_types.Vector.map2 ~f t1.coefficients_comm
          t2.coefficients_comm
    ; generic_comm = f t1.generic_comm t2.generic_comm
    ...
    ; lookup_table_comm =
        Kimchi_backend_types.Vector.map2 ~f:f_opt t1.lookup_table_comm

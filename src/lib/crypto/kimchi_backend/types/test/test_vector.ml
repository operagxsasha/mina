let test_initialize_with_correct_size () =
  let v =
    Kimchi_backend_types.Vector.init Kimchi_backend_types.Nat.N10.n ~f:(fun i ->
        i )
  in
  assert (Kimchi_backend_types.(Nat.to_int (Vector.length v)) = 10)
...
let test_split () =
  ...
  (* 6 + 4 *)
  let ten =
    snd (Kimchi_backend_types.Nat.N6.add Kimchi_backend_types.Nat.N4.n)
  in
  let v_6, v_4 = Kimchi_backend_types.Vector.split v ten in
  (* Checking the size of both splits *)
  assert (Kimchi_backend_types.(Nat.to_int (Vector.length v_6)) = 6) ;
  assert (Kimchi_backend_types.(Nat.to_int (Vector.length v_4)) = 4) ;
  (* We will now check the elements have been splitted correctly, we should have
     0 to 5 in v_6 and 6 to 9 in v_4 *)
  let v_6_list = Kimchi_backend_types.Vector.to_list v_6 in
  assert (List.for_all2_exn v_6_list (List.init 6 ~f:(fun i -> i)) ~f:Int.equal) ;
  let v_4_list = Kimchi_backend_types.Vector.to_list v_4 in
  assert (

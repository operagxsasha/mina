...
let signature ?(signature_kind = Mina_compile_config.signature_kind) =
...
let signature_legacy ?(signature_kind = Mina_compile_config.signature_kind) =
  match signature_kind with
  | Mainnet ->
...
let zkapp_body ?(chain = Mina_compile_config.signature_kind) = salt @@ zkapp_body ~chain

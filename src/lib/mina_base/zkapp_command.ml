...
module Call_forest = struct
  ...
  module type Digest_intf = sig
    module Account_update : sig
      ...
      module Checked : sig
        ...
        val create :
          ?chain:Mina_compile_config.signature_kind -> Account_update.Checked.t -> t

        val create_body :
          ?chain:Mina_compile_config.signature_kind -> Account_update.Body.Checked.t -> t
      end

      include Digest_intf.S_aux with type t := t and type checked := Checked.t

      val create : ?chain:Mina_compile_config.signature_kind -> Account_update.t -> t

      val create_body :
        ?chain:Mina_compile_config.signature_kind -> Account_update.Body.t -> t
    end
  ...
  module Make_digest_str
    ...
    module Account_update = struct
      ...
      let create : ?chain:Mina_compile_config.signature_kind -> Account_update.t -> t =

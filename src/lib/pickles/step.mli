...
module Make
    (A : Kimchi_backend_types.Poly_types.T0) (A_value : sig
      type t
    end)
    (Max_proofs_verified : Kimchi_backend_types.Nat.Add.Intf_transparent) : sig
  val f :
       ?handler:
    ...
    -> prevs_length:
         ('prev_vars, 'prevs_length) Kimchi_backend_types.Hlist.Length.t
    -> self:('a, 'b, 'c, 'd) Tag.t
    -> step_domains:
         (Import.Domains.t, 'self_branches) Kimchi_backend_types.Vector.t
         Promise.t
    ...
    -> ( ( 'value
       ...
       * 'ret_value
       * 'auxiliary_value
       * (int, 'prevs_length) Kimchi_backend_types.Vector.t )
       Promise.t
end

...
module Basic : sig
  type ('var, 'value, 'n1, 'n2) t =
    { max_proofs_verified :
        (module Kimchi_backend_types.Nat.Add.Intf with type n = 'n1)
    ; public_input : ('var, 'value) Impls.Step.Typ.t
    ; branches : 'n2 Kimchi_backend_types.Nat.t
...
module Compiled : sig
  type ('a_var, 'a_value, 'max_proofs_verified, 'branches) basic =
    ...
    ; wrap_domains : Import.Domains.t
    ; step_domains : (Import.Domains.t, 'branches) Kimchi_backend_types.Vector.t
    ; feature_flags : Opt.Flag.t Kimchi_backend_common.Plonk_types.Features.Full.t
    ; num_chunks : int
    ; zk_rows : int
    }
...
module For_step : sig
  type ('a_var, 'a_value, 'max_proofs_verified, 'branches) t =
    ...
    ; proofs_verifieds :
        [ `Known of
          (Impls.Step.Field.t, 'branches) Kimchi_backend_types.Vector.t
        | `Side_loaded ]
    ; public_input : ('a_var, 'a_value) Impls.Step.Typ.t
    ; wrap_key :
        inner_curve_var array Kimchi_backend_common.Plonk_verification_key_evals.t
    ; wrap_domain :
...
val feature_flags : _ Tag.t -> Opt.Flag.t Kimchi_backend_common.Plonk_types.Features.Full.t

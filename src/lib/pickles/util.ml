...
let rec absorb :
 ...
 fun ~absorb_field ~absorb_scalar ~g1_to_field_elements ~mask_g1_opt ty t ->
  ...
  | Type.With_degree_bound ->
      let Kimchi_backend_common.Plonk_types.Poly_comm.With_degree_bound.
            { unshifted; shifted } =
        t
      in
      let absorb x =
 ...
 fun ~first_zero (module Impl) n ->
  ...
  let rec go :
   ...
   fun value i m ->
    ...
    | Kimchi_backend_types.Nat.Z ->
        Kimchi_backend_types.Vector.[]
    | Kimchi_backend_types.Nat.S m ->
        let value =
          Boolean.(value && not (Field.equal first_zero (Field.of_int i)))
        in
        Kimchi_backend_types.Vector.(value :: go value (i + 1) m)
  in

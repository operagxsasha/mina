module Data : sig
  module Stable : sig
    module V1 : sig
      type t = { constraints : int } [@@deriving yojson]

      include Kimchi_backend_types.Sigs.VERSIONED
    end
  end

  type t = Stable.V1.t = { constraints : int } [@@deriving yojson]
end

module Stable : sig
  module V2 : sig
    type t =
      { commitments :
          Backend.Tock.Curve.Affine.t
          Kimchi_backend_common.Plonk_verification_key_evals.t
      ; index : Impls.Wrap.Verification_key.t
      ; data : Data.t
      }
    [@@deriving fields, to_yojson, of_yojson, bin_shape, bin_io]

    include Kimchi_backend_types.Sigs.VERSIONED
  end

  module Latest = V2
end

type t = Stable.Latest.t =
  { commitments :
      Backend.Tock.Curve.Affine.t Kimchi_backend_common.Plonk_verification_key_evals.t
  ; index : Impls.Wrap.Verification_key.t
  ; data : Data.t
  }
[@@deriving fields, to_yojson, of_yojson]

val dummy_commitments : 'a -> 'a Kimchi_backend_common.Plonk_verification_key_evals.t

val dummy_step_commitments :
  'a -> ('a, 'a option) Kimchi_backend_common.Plonk_verification_key_evals.Step.t

val dummy : Stable.Latest.t lazy_t
